...
import {
  AlterTableAddColumnSql, AlterTableAddForeignKeySql, AlterTableDropColumnSql, AlterTableDropConstraintSql,
  isAlterTableAddColumnSql,
  isAlterTableAddForeignKeySql, isAlterTableDropColumnSql, isAlterTableDropConstraintSql,
...
export class AlterTableDropConstraintFormatter implements FormatHandle<AlterTableDropConstraintSql> {
  ...
  handle(param: AlterTableDropConstraintSql, ctx: FormatContext, formatter: Formatter): string {
    return `ALTER TABLE ${formatter.format(param.alterTable, ctx)} DROP CONSTRAINT ${ctx.escape(param.drop.constraint)}`;
  }
}
...
export class AlterTableAddForeignKeyFormatter implements FormatHandle<AlterTableAddForeignKeySql<any>> {
  ...
  handle(param: AlterTableAddForeignKeySql<any>, ctx: FormatContext, formatter: Formatter): string {
    ...
    sql += ` REFERENCES ${formatter.format(param.add.references.table, ctx)} (${this.formatKeys(param.add.references.primaryKeys, ctx)})`;
    return sql;
  }
