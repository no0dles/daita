...
async function testDelete<T>(
  sqlTable: SqlTable,
  action: (builder: RelationalDeleteBuilder<T>) => RelationalDeleteBuilder<T>,
  expectedQueryOrError: SqlDelete | Error,
) {
  const relationalAdapter = new RelationalAdapterMock();
  const builder = new RelationalDeleteBuilder<T>(relationalAdapter, {
    delete: sqlTable,
  });
...
describe('builder/relational-delete-builder', () => {
  it('should delete({table: User}).where({foo: 1, bar: test})', async () => {
    await testDelete(
      { table: 'User' },
      (builder) => builder.where({ foo: 1, bar: 'test' }),
      {
        delete: { table: 'User' },
        where: {
          and: [
  ...
  it('should not allow update({table: User}).where({parent:{foo: 2}})', async () => {
    await testDelete(
      { table: 'User' },
      (builder) => builder.where({ parent: { foo: 2 } }),
