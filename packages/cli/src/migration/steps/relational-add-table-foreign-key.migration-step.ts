...
export class ExtendedRelationalAddTableForeignKey
  extends RelationalAddTableForeignKey
  implements ExtendedMigrationStep {
  get className() {
    return RelationalAddTableForeignKey.name;
  }
  toNode(): ts.NewExpression {
    return ts.createNew(
      ...
      [
        ...
        ts.createArrayLiteral(
          this.foreignFieldNames.map(fieldName =>
            ts.createStringLiteral(fieldName),
          ),
        ),
      ],
    );
  ...
  static parse(args: ts.Expression[]): RelationalAddTableForeignKey | null {
    ...
    const foreignFieldNameArray = isKind(
      args[4],
      ts.SyntaxKind.ArrayLiteralExpression,
    );
    ...
    const foreignFieldNames: string[] = foreignFieldNameArray.elements.map(
      elm => {
        const fieldName = isKind(elm, ts.SyntaxKind.StringLiteral);
        if (!fieldName) {
          throw new Error('not string');
    ...
    return new RelationalAddTableForeignKey(
      table.text,
      name.text,
      fieldNames,
      foreignTable.text,
