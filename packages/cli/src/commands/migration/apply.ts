...
export default class Apply extends Command {
  ...
  async run() {
    ...
    const migrationSchema = getMigrationSchema(schemaInfo.migrationTree.path(last[0].id));
    const context = new RelationalContext(migrationSchema, schemaInfo.migrationTree, dataAdapter);
