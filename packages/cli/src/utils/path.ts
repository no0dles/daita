...
export async function getSchemaLocation(flags: { schema: string | undefined }, cmd: Command): Promise<SchemaLocation> {
  ...
  if (fs.existsSync(tsconfigFileName)) {
    ...
    if (tsconfig.compilerOptions && tsconfig.compilerOptions.rootDir) {
      sourceDirectory = path.join(process.cwd(), tsconfig.compilerOptions.rootDir);
    }
  }
  ...
  if (flags.schema) {
    ...
    if (fs.existsSync(fileName)) {
      return resolveSchemaLocation(fileName, sourceDirectory, migrationDirectory);
    } else {
  ...
  let fileName = path.relative(process.cwd(), path.join(sourceDirectory, 'schema.ts'));
  while (true) {
    fileName = await cli.prompt('Where is your schema file?', {default: fileName});
    if (!fs.existsSync(path.join(process.cwd(), fileName))) {
      cmd.warn(`schema not found at ${fileName}`);
      continue;
    }

    return resolveSchemaLocation(path.join(process.cwd(), fileName), sourceDirectory, migrationDirectory);
  }
}

function resolveSchemaLocation(fileName: string, sourceDirectory: string, migrationDirectory: string): SchemaLocation {
  return {
    fileName: fileName,
...
export async function getSchemaInformation(location: SchemaLocation, cmd: Command): Promise<SchemaInformation | null> {
  ...
  const {migrationTree, migrationFiles} = parseSchemaMigrations(sourceFile, schema.name);

  return {migrationFiles, migrationTree, modelSchema, variableName: schema.name};
}
