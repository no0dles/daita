...
async function testSelect<T = any>(sqlTable: SqlTable, action: (builder: RelationalSelectBuilder<T>) => RelationalSelectBuilder<T>, expectedQueryOrError: SqlSelect | Error) {
  const relationalAdapter = new RelationalAdapterMock();
  const builder = new RelationalSelectBuilder<T>(relationalAdapter, sqlTable);
...
describe('builder/relational-select-builder', () => {
  ...
  it('should select({table: User}).join({table: User}, parent, on.eq(parentId, id))', async () => {
    await testSelect({table: 'User'}, builder => builder.join({table: 'User'}, 'parent', on => on.eq(u => u.parentId, p => p.parent.id)), {
      select: [
        {table: 'User', all: true},
