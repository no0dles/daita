...
async function testInsert<T>(sqlTable: SqlTable, action: (builder: RelationalInsertBuilder<T>) => RelationalInsertBuilder<T>, expectedQuery: SqlInsert) {
  const relationalAdapter = new RelationalAdapterMock();
  relationalAdapter.expect(expectedQuery);

  const builder = new RelationalInsertBuilder<T>(relationalAdapter, sqlTable);
  await action(builder);
}

describe('builder/relational-insert-builder', () => {
  it('should insert(User).value({foo: 1, bar: test})', async () => {
    await testInsert(User, builder => builder.value({foo: 1, bar: 'test'}), {
  ...
  it('should insert(User).values({foo: 1, bar: test})', async () => {
    await testInsert(User, builder => builder.values({foo: 1, bar: 'test'}, {bar: 'test'}), {
      insert: {table: User.table, schema: User.schema},
