...
async function testUpdate<T>(sqlTable: SqlTable, action: (builder: RelationalUpdateBuilder<T>) => RelationalUpdateBuilder<T>, expectedQueryOrError: SqlUpdate | Error) {
  const relationalAdapter = new RelationalAdapterMock();
  const builder = new RelationalUpdateBuilder<T>(relationalAdapter, sqlTable);
...
describe('builder/relational-update-builder', () => {
  it('should update({table: User}).set({foo: 1, bar: test})', async () => {
    await testUpdate({table: 'User'}, builder => builder.set({foo: 1, bar: 'test'}), {
      update: {table: 'User'},
      set: {foo: 1, bar: 'test'},
    });
  });
  ...
  it('should update({table: User}).set({foo: 1, bar: test}).where({foo: 2})', async () => {
    await testUpdate({table: 'User'}, builder => builder.set({foo: 1, bar: 'test'}).where({foo: 2}), {
      update: {table: 'User'},
      set: {foo: 1, bar: 'test'},
  ...
  it('should not allow update({table: User}).set({foo: 1, bar: test}).where({parent:{foo: 2}})', async () => {
    await testUpdate({table: 'User'}, builder => builder.set({foo: 1, bar: 'test'}).where({parent: {foo: 2}}), new Error('not supported yet'));
  });
});