...
export class RelationalUpdateContext<T> {
  ...
  where(filter: RootFilter<T>): RelationalUpdateContext<T> {
    if (this.filter) {
      return new RelationalUpdateContext<T>(this.dataAdapter, this.schema, this.type, this.data, {
        $and: [this.filter, filter],
      });
    }

    return new RelationalUpdateContext<T>(this.dataAdapter, this.schema, this.type, this.data, filter);
  }
  ...
  async exec(): Promise<{ affectedRows: number }> {
    ...
    return this.dataAdapter.update(this.schema, this.type.name, this.data, this.filter);
  }
}
