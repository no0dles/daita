...
export class RelationalDeleteContext<T> {
  ...
  where(data: RootFilter<T>): RelationalDeleteContext<T> {
    if (this.filter) {
      return new RelationalDeleteContext<T>(this.dataAdapter, this.schema, this.type, {
        $and: [this.filter, data],
      });
    }

    return new RelationalDeleteContext<T>(this.dataAdapter, this.schema, this.type, data);
  }

  async exec(): Promise<{ affectedRows: number }> {
    const result = await this.dataAdapter.delete(this.schema, this.type.name, this.filter);
    return result;
  }
}
