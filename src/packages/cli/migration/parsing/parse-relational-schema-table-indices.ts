  ...
  for (const indexProperty of indexObject.props) {
    ...
    if (indexValue instanceof AstArrayValue) {
      addIndex(
        table,
        indexProperty.name,
        getArrayValue(indexValue, (elm) => getStringValue(elm)),
        false,
      );
      ...
      if (columnsProp instanceof AstArrayValue) {
        const columns = getArrayValue(columnsProp, (elm) =>
          getStringValue(elm),
        );
        const unique = indexValue.hasProp('unique')
          ? indexValue.booleanProp('unique')
          : false;
