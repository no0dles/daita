...
describe('cli migration:add', () => {
  ...
  it(`should support inline migration`, setupEnv('add-inline-migration', async (ctx) => {
  ...
  it(`should add empty migration`, setupEnv('add-empty-migration', async (ctx) => {
    await ctx.run('migration:add second').finished;
    await ctx.exists('src/migrations');
    await ctx.notExists('src/migrations', /[0-9]+\-second.ts/);
  }, { schema: 'auth-schema-migrated' }));

  it(`should add second migration`, setupEnv('add-second-migration', async (ctx) => {
