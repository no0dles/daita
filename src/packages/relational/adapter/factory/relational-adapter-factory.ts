...
import { CreateDataAdapterOptions, isCreateDataAdapterOptionsSingle } from "./create-data-adapter-options";
import {
...
export function createDataAdapter(options: CreateDataAdapterOptions) {
  ...
  if (isCreateDataAdapterOptionsSingle(options)) {
    ...
    const adapter = detectAdapter(connectionString, options.adapters.map(a => a.adapterFactory));
...
function getConnectionString(options: CreateAdapterOptions) {
  ...
  if (!connectionString) {
    throw new Error("unable to detect a connection string, please specify on in the options or provide the environment variable DATABASE_URL");
  }
  return connectionString;
}

function detectAdapter<TAdapter extends RelationalAdapterFactory>(connectionString: string, adapters: TAdapter[]): TAdapter {
  const availableAdapters = adapters.filter(a => a.canCreate(connectionString));
  if (availableAdapters.length === 0) {
    throw new Error(`none of the specified adapters is able create [${adapters.map(a => a.name).join(", ")}]`);
  } else if (availableAdapters.length > 1) {
    throw new Error(`multiple adapters are configured [${availableAdapters.map(a => a.name).join(", ")}]`);
  }
