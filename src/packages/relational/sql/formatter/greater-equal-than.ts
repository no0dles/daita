...
import { GreaterEqualThanDescription, isGreaterEqualThanDescription } from '../description';
...
export class GreaterEqualThanFormatter implements FormatHandle<GreaterEqualThanDescription<any>> {
  ...
  handle(param: GreaterEqualThanDescription<any>, ctx: FormatContext, formatter: Formatter): string {
    return `${formatter.format(param.greaterEqualThan.left, ctx)} >= ${formatter.format(param.greaterEqualThan.right, ctx)}`;
  }
}
