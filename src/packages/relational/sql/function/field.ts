...
export function field<T, K extends keyof PickByValue<T, ValueType>>(typeOrAlias: Constructable<T> | TableAliasDescription<T> | TableDescription<T>, key: K): T[K] {
  if (typeof typeOrAlias === 'function') {
    return <FieldDescription>{ field: { key, table: table(typeOrAlias) } } as any;
  } else {
